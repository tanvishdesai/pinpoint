...
class Config:
    ...
    W_OFFSET = 0.5
    # --- ABLATION STUDY ---
    # W_ATTENTION = 3.0 # Removed as it corresponds to the custom loss

    # --- NEW: Synchronization Loss Parameters ---
    # --- ABLATION STUDY ---
    # This entire section is removed as it's specific to SynchronizationLoss.
...
def train_one_epoch(model, dataloader, optimizer, scheduler, loss_fns, device, config, epoch, scaler):
    ...
    for batch in progress_bar:
        ...
        with autocast():
            ...
            # The curriculum logic for phase 2 remains, affecting the offset loss
            if phase == 2:
                loss_offset = loss_offset * 0.1

            # --- ABLATION STUDY MODIFICATION ---
            # The 'loss_attn' calculation and its associated weight are removed.
        ...
        progress_bar.set_postfix({"Phase": phase, "Loss": f"{combined_loss.item():.4f}", "Acc": f"{correct_preds/total_samples:.2f}"})
...
if __name__ == '__main__':
    ...
    ### END MODIFIED BLOCK ###
        ...
        try:
            ...
            # --- ABLATION STUDY MODIFICATION ---
            # The custom SynchronizationLoss ('attention') has been removed.
            loss_fns = {
